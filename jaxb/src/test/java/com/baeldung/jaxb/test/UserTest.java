...
import com.baeldung.jaxb.gen.UserRequest;
...
public class UserTest {
    ...
    @Test
    public void unMashal2() throws JAXBException, IOException {

        JAXBContext jaxbcontext2;
        jaxbcontext2 = JAXBContext.newInstance(UserRequest.class);

        Unmarshaller unmarshaller = jaxbcontext2.createUnmarshaller();

        String userFile = this.getClass().getResource("/user.xml").getFile();
        UserRequest userRequest = (UserRequest) unmarshaller.unmarshal(new FileReader(userFile));
        //Assert.assertEquals(book, userRequest);
